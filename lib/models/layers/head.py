...
class Corner_Predictor_v3(nn.Module):
    ...
    def forward(self, x, pos_last, return_dist=False, softmax=True):
        """ Forward pass with input x. """
        score_map_tl, score_map_br = self.get_score_map(x)
    ...
    def soft_argmax_tl(self, score_map, return_dist=False, softmax=True):
        ...
        # img = np.array((score_map[0][0]).cpu())
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        elif cfg.MODEL.HEAD.TYPE == "CORNERv3":
            corner_head = Corner_Predictor_v3(inplanes=hidden_dim, channel=cfg.MODEL.HEAD.NUM_CHANNELS,
                                           feat_sz=feat_sz, stride=stride)
        else:
            raise ValueError()
        return corner_head
