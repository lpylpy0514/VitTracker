    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                import cv2
                x1, y1, w, h = template_anno['bbox'][0][0], template_anno['bbox'][0][1], template_anno['bbox'][0][2], template_anno['bbox'][0][3]
                cv2.rectangle(template_frames[-1], (int(x1), int(y1)), (int(x1 + w), int(y1 + h)), color=(255, 0, 0), thickness=3)
                # img = cv2.cvtColor(template_frames[-1], cv2.COLOR_BGR2RGB)
                # cv2.imshow('img', img)
                # cv2.waitKey(0)
