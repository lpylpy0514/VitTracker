...
class OSTrackDistillationActor(BaseActor):
    """ Actor for training OSTrack models """

    def __init__(self, teachernet, net, objective, loss_weight, settings, cfg=None):
    ...
    def forward_pass(self, data, net, mode):
        ...
        out_dict = {}
        if self.cfg.TRAIN.TEACHER == 'clip' and mode == 'teacher':
            feature = net([search_img, template_list])
            out_dict['res_list'] = feature
            return out_dict
        if self.cfg.TRAIN.TEACHER == 'MAE-L' and mode == 'teacher':
            feature = net([search_img, template_list])
            out_dict['res_list'] = feature
            return out_dict
        if mode == 'teacher':
            out_dict = net(template=template_list,
                                search=search_img,
